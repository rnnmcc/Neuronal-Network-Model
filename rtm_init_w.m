function rtm_init=rtm_init_w(i_ext,phi_vec,g_m)
...
%         contain values of v, h, and n and w. If i_ext(i) is below
...
while sum(done)<num && t<t_final
    ...
    for i=1:l
        ...
        rtm_init(k,4)=(w_old(k)*(t-thr(k))+w(k)*(thr(k)-t_old))/dt;
    end
    done(ind)=1;
